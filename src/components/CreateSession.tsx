...
const CreateSession: React.FC<props> = ({ smartAccount, address, provider, setIsSessionActive, isSessionActive }) => {
  ...
  return (
    <div>
      ...
      <button onClick={() => createSession(false)} >Enable Tipping</button>
      ) : (
        <button onClick={() => createSession(true)} >Enable Tipping</button>
      )}
      {/* {isSessionActive && <ERC20Transfer smartAccount={smartAccount} provider={provider} address={address} />} */}
      {isSessionActive && <p>Tipping enabled</p>}
    </div>
